...
export default function ActivityPicture() {
  ...
  useEffect(() => {
    const fetchActivityImages = async () => {
      try {
        const response = await instance.get("posts/main-picture");
        setActivityImages(response.data);
      } catch (error) {
      ...
      <Grid item xs={12}>
        <Grid container spacing={2}>
          {activityImages.map((image) => (
            <Grid item xs={12} sm={6} md={3} key={image.postId}>
              <Card>
                <CardMedia
                  component="img"
                  height="140"
                  image={`data:image/jpeg;base64,${image.photoBase64}`}
                  alt={image.title}
                />
                <CardContent>
